...
public class UserServiceTest implements OwnerConstants{
	...
	@Mock 
	TypedQuery<OwnerPortalPhoneDisclaimer> ownerPortalPhoneDisclaimerQuery;
...
//@Test
//@Ignore
public void validateCdiidTest() throws Exception {
		LOG.info("inside validateCdiidtest success case");
...
//@Test
//@Ignore
public void getPhoneDisclaimerTest() throws Exception {
		LOG.info("inside getPhoneDisclaimer test success case");

		// Setup test input
		String userProfileId = "1564";
					 
					 
		OwnerPortalPhoneDisclaimer ownerPortalPhoneDisclaimer = new OwnerPortalPhoneDisclaimer();
		ownerPortalPhoneDisclaimer.setMobilePhoneAutoDialerOpt("0");
		ownerPortalPhoneDisclaimer.setWorkPhoneAutoDialerOpt("1");

		List<OwnerPortalPhoneDisclaimer> ownerPortalPhoneDisclaimerrList = new ArrayList<OwnerPortalPhoneDisclaimer>();
		ownerPortalPhoneDisclaimerrList.add(ownerPortalPhoneDisclaimer);

		when(osEm.createNamedQuery("OwnerPortalPhoneDisclaimer.findByUserProfileId",OwnerPortalPhoneDisclaimer.class)).thenReturn(ownerPortalPhoneDisclaimerQuery);
		when(ownerPortalPhoneDisclaimerQuery.setParameter("userProfileId", userProfileId)).thenReturn(ownerPortalPhoneDisclaimerQuery);
		when(ownerPortalPhoneDisclaimerQuery.getResultList()).thenReturn(ownerPortalPhoneDisclaimerrList);
					
		OwnerPortalPhoneDisclaimer response = userService.getPhoneDisclaimer(userProfileId, osEm);
			        		
		LOG.info("Response for getPhoneDisclaimerTest" + response.getWorkPhoneAutoDialerOpt());

		Assert.assertEquals("0",response.getMobilePhoneAutoDialerOpt());
...
public void createLinkReferenceIdTest() throws Exception{
		 ...
		 Assert.assertEquals("1564",usrPrflId);
...
public void updateLinkReferenceHashIdStatusTest() throws Exception {
		...
		when(osEm.createQuery("Update OwnerPortalLinkReference set activeIn='N' where linkReferenceHashId= ? and userProfileId = ? ")).thenReturn(updateLinkRefQuery);
